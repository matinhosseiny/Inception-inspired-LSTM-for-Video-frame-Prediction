...
class PredNet(Recurrent):
    ...
    def get_initial_state(self, x):
        ...
        for u in states_to_pass:
            for l in range(nlayers_to_pass[u]):
                ...
                if u in ['r', 'c']:
                    stack_size = 3*self.R_stack_sizes[l]#matin
                elif u == 'e':
    ...
    def build(self, input_shape):
        ...
        self.conv_layers = {c: [] for c in ['i1', 'f1', 'c1', 'o1','i3', 'f3', 'c3', 'o3','i5', 'f5', 'c5', 'o5', 'o1', 'a', 'ahat']}

        for l in range(self.nb_layers):
            for c in ['i1', 'f1', 'c1', 'o1']:
                act = self.LSTM_activation if c == 'c1' else self.LSTM_inner_activation
                self.conv_layers[c].append(Conv2D(self.R_stack_sizes[l], 1, padding='same', activation=act, data_format=self.data_format))
            for c in ['i3', 'f3', 'c3', 'o3']:
                act = self.LSTM_activation if c == 'c3' else self.LSTM_inner_activation
                self.conv_layers[c].append(Conv2D(self.R_stack_sizes[l], 3, padding='same', activation=act, data_format=self.data_format))
            for c in ['i5', 'f5', 'c5', 'o5']:
                act = self.LSTM_activation if c == 'c5' else self.LSTM_inner_activation
                self.conv_layers[c].append(Conv2D(self.R_stack_sizes[l], 5, padding='same', activation=act, data_format=self.data_format))
            ...
            if l < self.nb_layers - 1:
                self.conv_layers['a'].append(Conv2D(self.stack_sizes[l+1], 3, padding='same', activation=self.A_activation, data_format=self.data_format))
        ...
        for c in sorted(self.conv_layers.keys()):
            for l in range(len(self.conv_layers[c])):
                ...
                if c == 'ahat':
                    ...
                    nb_channels = self.stack_sizes[l] * 2 + 3*self.R_stack_sizes[l]
                    if l < self.nb_layers - 1:
                        nb_channels += self.R_stack_sizes[l+1]*3
    ...
    def step(self, a, states):
        ...
        for l in reversed(range(self.nb_layers)):
            ...
            s=self.conv_layers['i1'][l].call(inputs)
            print("s",s.shape)
            i = K.concatenate([s,self.conv_layers['i3'][l].call(inputs)], axis=self.channel_axis)
            i = K.concatenate([i,self.conv_layers['i5'][l].call(inputs)], axis=self.channel_axis)
            print("i",i.shape)
            f = K.concatenate([self.conv_layers['f1'][l].call(inputs),self.conv_layers['f3'][l].call(inputs)], axis=self.channel_axis)
            f = K.concatenate([f,self.conv_layers['f5'][l].call(inputs)], axis=self.channel_axis)
            print("f",f.shape)
            o = K.concatenate([self.conv_layers['o1'][l].call(inputs),self.conv_layers['o3'][l].call(inputs)], axis=self.channel_axis)
            o = K.concatenate([o,self.conv_layers['o5'][l].call(inputs)], axis=self.channel_axis)
            print("o",o.shape)
            g = K.concatenate([self.conv_layers['c1'][l].call(inputs),self.conv_layers['c3'][l].call(inputs)], axis=self.channel_axis)
            g = K.concatenate([g,self.conv_layers['c5'][l].call(inputs)], axis=self.channel_axis)
            print("g",g.shape)
            _c = f * c_tm1[l] + i * g
